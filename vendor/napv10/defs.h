...
{
    // 🦅 FIX "mate -0": testar o RANGE de mate real [SCORE_MATE_IN_MAX, SCORE_MATE], não só
    //   ">= SCORE_MATE_IN_MAX". Antes, SCORE_NONE (-32701) passava o teste (|.|=32701 > 32572)
    //   → um displayScore por inicializar era lido como mate → reportava "mate -0".
    //   Agora SCORE_NONE/SCORE_MAX (fora do range de mate) NÃO são considerados mate.
    return std::abs(score) >= SCORE_MATE_IN_MAX && std::abs(score) <= SCORE_MATE;
}
