...
This section previously read "no source file is a port, a translation, or a
copy of another engine's code", and the table below was empty. Both were true
when written, and both stopped being true when the engine moved to the SFNNv16
format. That work has not been published before this change, so the licence
and this file are corrected in the same commit that first publishes it.
...
| Stockfish — phase-1 training data (5000-node evaluations) | not in this repo; used to train the shipped network | data, published by the Stockfish project | The positions and evaluations the network is trained on. |
...
The data, however, is **not** ours. This section previously said the network
was trained "on positions generated by this engine's own self-play" with "no
third-party dataset currently used". That is no longer the case: the network
is trained on the **Stockfish project's published phase-1 data** — positions
evaluated at 5000 nodes — filtered by this project's own pipeline. Credit for
that data belongs to the Stockfish project and to the people who generated it.
